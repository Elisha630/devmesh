            ...
            try:
                ...
                if self.metrics.last_pong_received is not None:
                    ...
                    if time_since_pong > self.ping_timeout:
                        log.warning(
                            f"WebSocket {self.client_id}: No pong received for {time_since_pong:.1f}s"
                        )
...
class HealthMonitor:
    ...
    def get_all_metrics(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {cid: health.metrics.to_dict() for cid, health in self.connections.items()}
    ...
    def get_unhealthy_connections(self) -> list[str]:
        ...
        return [cid for cid, health in self.connections.items() if not health.is_healthy()]
    ...
    async def cleanup_dead_connections(self, timeout_sec: float = 300) -> int:
        ...
        for cid, health in self.connections.items():
            ...
            if time_since_activity > timeout_sec:
                ...
                log.info(
                    f"Cleaning up dead connection: {cid} (inactive for {time_since_activity:.0f}s)"
                )
