...
class ServerConfig:
    ...
    # Dashboard
    auto_open_browser: bool = os.getenv("DEVMESH_AUTO_OPEN_BROWSER", "true").lower() in (
        "true",
        "1",
        "yes",
    )
    ...
    # UI history sampling
    hardware_sample_interval_sec: float = float(
        os.getenv("DEVMESH_HARDWARE_SAMPLE_INTERVAL_SEC", "2")
    )
    hardware_history_len: int = int(os.getenv("DEVMESH_HARDWARE_HISTORY_LEN", "120"))

    # Conflict resolution rate limiting
    conflict_resolve_cooldown_sec: int = int(
        os.getenv("DEVMESH_CONFLICT_RESOLVE_COOLDOWN_SEC", "15")
    )
...
TOOL_PROFILES: Dict = {
    ...
    "gemini": {
        ...
        "capabilities": {
            "languages": ["python", "javascript", "go", "java", "kotlin"],
    ...
    "cody": {
        ...
        "color": "#FF5543",
        "invoke_mode": "arg",
        "cmd": ["cody", "chat", "--no-confirm", "--message", "{prompt}"],
        "capabilities": {
    ...
    "gh": {
        ...
        "color": "#238636",
        "invoke_mode": "arg",
        "cmd": ["gh", "copilot", "suggest", "--target", "shell", "--no-cache", "{prompt}"],
        "capabilities": {
...
assert set(t["name"] for t in KNOWN_CLI_TOOLS) == set(
    TOOL_PROFILES.keys()
